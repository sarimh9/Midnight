...
//* ==================== API Functionality ====================
const sunriseApi = document.querySelector(".sunrise-api");
const sunsetApi = document.querySelector(".sunset-api");
const midnightApi = document.querySelector(".midnight-api");

let date = new Date();
let d = date.getDate();
let m = date.getMonth() + 1;
let y = date.getFullYear();
const API = `https://api.aladhan.com/v1/timingsByCity/${d}-${m}-${y}?city=Meerut&country=India`;
...
async function setData() {
  ...
  sunsetTimeEle.value = data.data.timings.Sunrise;
  sunriseTimeEle.value = data.data.timings.Sunset;

  sunriseApi.textContent = data.data.timings.Sunrise;
  sunsetApi.textContent = data.data.timings.Sunset;
  midnightApi.textContent = data.data.timings.Midnight;
}
